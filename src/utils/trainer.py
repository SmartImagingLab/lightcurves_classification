...
class Trainer():
    ...
    def train_epoch(self, epoch):
        ...
        for iteration, data in enumerate(self.traindataloader):
            ...
            # criterion = F.CrossEntropyLoss(weight=weight, reduction='none')
            bitch_weight = 1
            F_weight = F.nll_loss(weight=self.classweights, reduction='none')
            loss = F_weight(logprobabilities, targets[:, 0])
            loss = loss * bitch_weight
            # loss.mean().backward()
